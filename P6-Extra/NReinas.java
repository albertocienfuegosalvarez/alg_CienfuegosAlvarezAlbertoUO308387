...
public class NReinas {
    ...
    private void inicializarRastreadores(int n) {
        columnas = new boolean[n];
        diagonalesPrincipales = new boolean[2*n - 1];
        diagonalesSecundarias = new boolean[2*n - 1];
        // False significa que si se puede meter, es decir que no hay reina
        for (int i = 0; i < columnas.length; i++) {
            columnas[i] = false;
        } 

        for (int i = 0; i < diagonalesPrincipales.length; i++) {
            diagonalesPrincipales[i] = false;
            diagonalesSecundarias[i] = false;
        }
    }
